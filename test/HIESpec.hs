...
spec = do
  ...
  describe "with" do
    it "loads dependencies asynchronously" do
  ...
  describe "loadPackages" do
    it "reads exports from a list of HIE files" do
      files <- filter (isInfixOf "markdown-unlit") . snd <$> listAllHieFiles info
      exports <- newIORef mempty
      _ <- loadPackages files (modifyIORef exports)
    ...
    context "with a missing HIE file" do
      it "reports an error" do
        withTempDirectory \ dir -> do
          ...
          loadPackages [file] mempty <&> (.errors) `shouldReturn` [unlines [
              "Reading " <> file <> " failed:"
