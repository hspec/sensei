...
parseAnnotation diagnostic = case diagnostic.code of
  ...
  Just 62161 -> parseNonExhaustivePatterns
  _ -> analyzeMessage
  where
    parseNonExhaustivePatterns :: Maybe Annotation
    parseNonExhaustivePatterns = do
      m <- head diagnostic.message
      case T.lines m of
        "Pattern match(es) are non-exhaustive" : xs -> case xs of
          "In a \\case alternative:" : type_ : patterns -> nonExhaustivePatterns type_ patterns
          "In a case alternative:" : type_ : patterns -> nonExhaustivePatterns type_ patterns
          _ -> Nothing
        _ -> Nothing

    nonExhaustivePatterns :: Text -> [Text] -> Maybe Annotation
    nonExhaustivePatterns type_ patterns = do
      name <- stripPrefix "    Patterns of type `" type_ >>= stripSuffix "' not matched:"
      return $ NonExhaustivePattern name $ mapMaybe (stripPrefix "        ") patterns
...
analyzeAnnotation availableImports diagnostic = \ case
  ...
  NonExhaustivePattern _name patterns -> [AddPatterns patterns]
  where
