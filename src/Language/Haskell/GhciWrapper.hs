...
import           GHC.Diagnostic (Annotated, FormatConfig(..))
...
, parseMessage = \ input -> do
    Diagnostic.parseAnnotated getAvailableImports input >>= \ case
      ...
      Just diagnostic -> do
        ...
        let (next, formatted) = Diagnostic.formatAnnotated formatConfig n diagnostic
...
extractDiagnostics = ReadHandle.Extract {
  isPartialMessage = ByteString.isPrefixOf "{"
, parseMessage = \ input -> return $ (id &&& encodeUtf8 . Diagnostic.format formatConfig) <$> Diagnostic.parse input
}
