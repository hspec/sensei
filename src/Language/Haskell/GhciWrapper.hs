...
, echo :: ByteString -> IO ()
, identifierMap :: IORef Diagnostic.IdentifierMap
}
...
extractReloadDiagnostics :: Diagnostic.IdentifierMap -> Extract (Either ReloadStatus Annotated)
extractReloadDiagnostics identifierMap = extractReloadStatus <+> extractDiagnostics identifierMap
...
extractDiagnostics :: Diagnostic.IdentifierMap -> ReadHandle.Extract Annotated
extractDiagnostics identifierMap = ReadHandle.Extract {
  isPartialMessage = ByteString.isPrefixOf "{"
, parseMessage = fmap (id &&& encodeUtf8 . Diagnostic.formatAnnotated) . Diagnostic.parseAnnotated identifierMap
}
