...
new startupFile Config{..} envDefaults args_ = do
  ...
  let
    ...
    mandatoryArgs = diagnosticsAsJson [
        "--interactive"
...
(<+>) a b = Extract {
  isPartialMessage = (||) <$> a.isPartialMessage <*> b.isPartialMessage
, parseMessage = \ x -> first Left <$> a.parseMessage x <|> first Right <$> b.parseMessage x
}
