...
run args = withSystemTempDirectory "sensei" \ hieDir -> do
  ...
  config <- loadConfig
  runArgs@RunArgs{dir, lastOutput, queue, cleanupAction, moduleIndex} <- defaultRunArgs (Just hieDir)
  ...
  _ <- forkIO do -- wait for thread, don't delegate control-c?
    Session.withSession c [] \ session -> do
      ...
      ((), dt) <- timeAction $ for_ modules \ mod -> do
        ...
        atomicModifyIORef' moduleIndex.globalIdentifierMap (insertAll &&& const ())
      atomicWriteIORef moduleIndex.globalIdentifierMapConstructionTime dt
  ...
  let
    ...
    , getModules = Set.toList . mappend globalModules <$> atomicReadIORef moduleIndex.modules
    }
  ...
  HTTP.withApp appConfig do
    watch do
      ...
      runWith runArgs { config, args, moduleIndex = moduleIndex { globalModules } }
  where
...
defaultRunArgs hieDir = do
  ...
  , args = []
  , lastOutput
  , moduleIndex = ModuleIndex mempty modules globalIdentifierMap globalIdentifierMapConstructionTime
  , queue = queue
    ...
    go onTestRunStarted extraArgs = do
      status <- withSession sessionConfig (extraArgs <> args) $ \ session -> do
        let
          ...
          triggerAction = saveOutput (trigger moduleIndex session hooks <* printExtraArgs)

          triggerAllAction :: OnTestRunStarted -> IO ()
          triggerAllAction = saveOutput (triggerAll moduleIndex session hooks <* printExtraArgs)
